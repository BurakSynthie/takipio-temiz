      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brandCapsule {
          ...
          padding: 9px 17px;
          border-radius: 22px;
          background: linear-gradient(180deg, rgba(7, 15, 32, 0.96), rgba(5, 9, 20, 0.96)), #050914;
          border: 1px solid rgba(255, 255, 255, 0.14);
          box-shadow: 0 18px 38px rgba(6, 16, 31, 0.22), inset 0 0 22px rgba(11, 99, 255, 0.1);
        }

        .brandCapsule img { width: 154px; height: 42px; object-fit: contain; filter: drop-shadow(0 0 12px rgba(11, 99, 255, 0.3)); }

        .navLinks { display: flex; align-items: center; gap: 27px; color: #1d2939; font-size: 15px; font-weight: 850; }
        .navLinks a { transition: 0.24s ease; }
        ...
        .statusPill span { width: 9px; height: 9px; border-radius: 50%; background: #22c55e; box-shadow: 0 0 0 0 rgba(34, 197, 94, 0.55); animation: pulse 1.8s ease-in-out infinite; }

        h1 { max-width: 780px; font-size: clamp(56px, 5.4vw, 92px); line-height: 0.96; letter-spacing: -5px; font-weight: 950; color: var(--ink); margin-bottom: 23px; }
        h1 mark { color: var(--blue); background: transparent; text-shadow: 0 12px 30px rgba(11, 99, 255, 0.16); }
        .heroText { max-width: 650px; color: var(--muted); font-size: 21px; line-height: 1.58; letter-spacing: -0.4px; margin-bottom: 28px; }
        ...
        .waitlistCard:hover { transform: translateY(-4px); box-shadow: 0 38px 90px rgba(15, 32, 64, 0.16), 0 0 0 6px rgba(11, 99, 255, 0.035); }
        .waitlistHeader { display: flex; align-items: center; gap: 16px; margin-bottom: 18px; }
        .mailIcon { width: 56px; height: 56px; flex: 0 0 auto; border-radius: 19px; display: grid; place-items: center; color: white; background: linear-gradient(135deg, var(--blue), var(--blue2)); box-shadow: 0 16px 30px rgba(11, 99, 255, 0.24); }
        .mailIcon svg { width: 28px; height: 28px; }
        .waitlistHeader h2 { font-size: 25px; line-height: 1.1; letter-spacing: -0.9px; margin-bottom: 6px; }
        ...
        .formRow input { height: 59px; border: 1px solid rgba(11, 99, 255, 0.18); border-radius: 18px; outline: none; background: #f7fbff; color: var(--ink); padding: 0 18px; font-size: 16px; transition: 0.25s ease; }
        .formRow input:focus { border-color: rgba(11, 99, 255, 0.48); box-shadow: 0 0 0 5px rgba(11, 99, 255, 0.08); background: white; }
        .formRow button { height: 59px; border: 0; border-radius: 18px; background: linear-gradient(135deg, var(--blue), var(--blue2)); color: white; font-size: 17px; font-weight: 950; box-shadow: 0 16px 28px rgba(11, 99, 255, 0.24); transition: 0.25s ease; display: flex; align-items: center; justify-content: center; gap: 9px; }
        .formRow button:hover { transform: translateY(-2px); box-shadow: 0 22px 38px rgba(11, 99, 255, 0.3); }
        ...
        .proofBar div { min-height: 82px; border-radius: 24px; padding: 15px; background: rgba(255, 255, 255, 0.68); border: 1px solid rgba(11, 99, 255, 0.12); box-shadow: 0 16px 35px rgba(7, 20, 47, 0.06); transition: 0.25s ease; }
        .proofBar div:hover { transform: translateY(-4px); border-color: rgba(11, 99, 255, 0.3); background: white; }
        ...
        .ambientGlow { position: absolute; width: 690px; height: 420px; border-radius: 50%; background: radial-gradient(ellipse at center, rgba(11, 99, 255, 0.12), transparent 64%); transform: rotate(-8deg); }

        .laptopMock { width: min(670px, 94%); height: 405px; position: relative; border-radius: 32px 32px 20px 20px; padding: 17px 17px 31px; background: linear-gradient(135deg, #c4cfdd, #ffffff 48%, #8d99aa); box-shadow: 0 42px 98px rgba(7, 20, 47, 0.22); transform: rotateX(4deg) rotateY(-8deg) rotateZ(-1deg); transition: 0.35s ease; z-index: 3; }
        ...
        .laptopMock::after { content: ""; position: absolute; left: 7%; right: 7%; bottom: -18px; height: 26px; border-radius: 0 0 36px 36px; background: linear-gradient(180deg, #e6ecf4, #7d8796); box-shadow: 0 18px 30px rgba(7, 20, 47, 0.16); }
        .laptopScreen { width: 100%; height: 100%; overflow: hidden; border-radius: 22px; background: #041020; border: 1px solid rgba(255, 255, 255, 0.12); box-shadow: inset 0 0 42px rgba(0, 168, 255, 0.09); text-align: left; }
        .screenTop { height: 56px; display: flex; align-items: center; justify-content: space-between; padding: 0 18px; color: white; border-bottom: 1px solid rgba(255, 255, 255, 0.08); }
        ...
        .screenActions span { width: 8px; height: 8px; border-radius: 50%; background: rgba(255, 255, 255, 0.3); }
        .screenBody { height: calc(100% - 56px); display: grid; grid-template-columns: 156px 1fr; }
        .mockMenu { padding: 16px 12px; border-right: 1px solid rgba(255, 255, 255, 0.08); display: grid; align-content: start; gap: 10px; background: rgba(0, 0, 0, 0.1); }
        .mockMenu button { min-height: 34px; border: 0; border-radius: 10px; background: rgba(255, 255, 255, 0.07); color: rgba(255, 255, 255, 0.66); font-size: 11px; font-weight: 800; transition: 0.22s ease; }
        .mockMenu button:hover, .mockMenu button.active { color: white; background: linear-gradient(135deg, var(--blue), var(--blue2)); box-shadow: 0 10px 20px rgba(11, 99, 255, 0.22); }
        .mockAssistant { margin-top: 12px; min-height: 68px; border-radius: 15px; padding: 11px; color: white; background: rgba(255, 255, 255, 0.06); border: 1px solid rgba(255, 255, 255, 0.08); }
        .mockAssistant b { display: block; font-size: 12px; margin-bottom: 5px; }
        .mockAssistant span { display: block; color: rgba(255, 255, 255, 0.58); font-size: 10px; line-height: 1.3; }
        ...
        .mockCard { min-height: 76px; padding: 12px; border-radius: 14px; background: rgba(255, 255, 255, 0.06); border: 1px solid rgba(255, 255, 255, 0.08); color: white; transition: 0.22s ease; }
        .mockCard:hover { background: rgba(255, 255, 255, 0.1); transform: translateY(-3px); }
        .mockCard small { display: block; color: #8fb7ff; font-size: 10px; margin-bottom: 8px; }
        .mockCard b { display: block; font-size: 18px; letter-spacing: -0.4px; }
        ...
        .mockGraph { height: 172px; border-radius: 16px; overflow: hidden; background: linear-gradient(180deg, rgba(11, 99, 255, 0.2), rgba(11, 99, 255, 0.03)); border: 1px solid rgba(255, 255, 255, 0.08); position: relative; }
        ...
        .activityPanel { height: 172px; border-radius: 16px; padding: 12px; background: rgba(255, 255, 255, 0.055); border: 1px solid rgba(255, 255, 255, 0.08); color: white; overflow: hidden; }
        .activityPanel h4 { font-size: 12px; margin-bottom: 10px; }
        .activityPanel div { min-height: 31px; display: flex; align-items: center; justify-content: space-between; gap: 8px; border-radius: 10px; padding: 0 9px; margin-bottom: 7px; background: rgba(255, 255, 255, 0.055); font-size: 10px; color: rgba(255, 255, 255, 0.75); }
        .activityPanel b { color: var(--green); }
        .mockList { margin-top: 11px; display: grid; gap: 8px; }
        .mockList div { min-height: 36px; display: flex; align-items: center; justify-content: space-between; padding: 0 12px; border-radius: 12px; color: rgba(255, 255, 255, 0.78); background: rgba(255, 255, 255, 0.055); border: 1px solid rgba(255, 255, 255, 0.06); font-size: 12px; }
        .mockList b { color: white; }

        .phoneMock { position: absolute; right: -18px; bottom: 44px; width: 174px; height: 348px; border-radius: 38px; padding: 10px; background: linear-gradient(135deg, #1c2533, #050b15); border: 3px solid #101827; box-shadow: 0 30px 60px rgba(7, 20, 47, 0.24); transform: rotate(3deg); transition: 0.35s ease; z-index: 7; }
        ...
        .phoneTabs button { height: 25px; border: 0; border-radius: 8px; background: rgba(255, 255, 255, 0.07); color: rgba(255, 255, 255, 0.62); font-size: 9px; font-weight: 800; }
        .phoneTabs button.active { color: white; background: var(--blue); }
        .phoneGrid { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; }
        .phoneCard { min-height: 62px; border-radius: 13px; padding: 9px; background: rgba(255, 255, 255, 0.07); border: 1px solid rgba(255, 255, 255, 0.08); }
        ...
        .phoneList div { height: 31px; border-radius: 11px; padding: 0 8px; display: flex; align-items: center; justify-content: space-between; background: rgba(255, 255, 255, 0.07); border: 1px solid rgba(255, 255, 255, 0.06); color: rgba(255, 255, 255, 0.7); font-size: 9px; }
        .phoneList b { color: var(--orange); }

        .floatingCard { position: absolute; z-index: 11; min-width: 178px; display: flex; align-items: center; gap: 12px; padding: 13px 15px; border-radius: 20px; background: rgba(255, 255, 255, 0.84); border: 1px solid rgba(11, 99, 255, 0.16); box-shadow: 0 24px 42px rgba(7, 20, 47, 0.13); transition: 0.28s ease; }
        .floatingCard:hover { transform: translateY(-8px) scale(1.02); border-color: rgba(11, 99, 255, 0.36); box-shadow: 0 34px 58px rgba(7, 20, 47, 0.18); }
        ...
        .floatRevenue { right: 82px; top: 50px; animation: floaty 5.8s ease-in-out infinite 0.35s; }
        .floatStock { left: 10px; bottom: 74px; animation: floaty 5.4s ease-in-out infinite 0.2s; }

        .sideStack { position: relative; z-index: 20; display: grid; gap: 18px; align-content: center; }
        .pricePanel { position: relative; overflow: hidden; min-height: 302px; border-radius: 34px; padding: 21px; color: white; background: radial-gradient(circle at 50% 0%, rgba(73, 217, 255, 0.28), transparent 36%), linear-gradient(180deg, #071026 0%, #050914 100%); border: 1px solid rgba(255, 255, 255, 0.13); box-shadow: 0 32px 74px rgba(7, 20, 47, 0.26); transition: 0.32s ease; }
        .pricePanel:hover { transform: translateY(-8px); box-shadow: 0 42px 90px rgba(7, 20, 47, 0.32); }
        .priceShine { position: absolute; inset: -70px auto auto -100px; width: 130px; height: 460px; background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.13), transparent); transform: rotate(28deg); animation: shine 4.8s ease-in-out infinite; }
        .priceLabel { width: max-content; margin: 0 auto 18px; padding: 10px 12px; border-radius: 13px; background: linear-gradient(135deg, var(--blue), var(--blue2)); font-size: 15px; font-weight: 950; box-shadow: 0 14px 25px rgba(11, 99, 255, 0.32); }
        .pricePanel p { text-align: center; text-transform: uppercase; color: rgba(255, 255, 255, 0.82); font-size: 15px; font-weight: 950; }
        .priceNumbers { text-align: center; margin: 5px 0 9px; }
        .priceNumbers del { display: block; color: rgba(255, 255, 255, 0.34); font-size: 42px; font-weight: 950; line-height: 1; text-decoration-color: var(--blue); text-decoration-thickness: 5px; }
        .priceNumbers strong { display: block; color: white; font-size: 74px; line-height: 0.95; text-shadow: 0 0 24px rgba(11, 99, 255, 0.9); }
        .pricePanel small { display: block; color: rgba(255, 255, 255, 0.62); text-align: center; font-size: 13px; font-weight: 750; margin-bottom: 14px; }
        .priceBullets { display: grid; gap: 8px; margin-bottom: 14px; }
        .priceBullets span { display: flex; align-items: center; gap: 7px; color: rgba(255, 255, 255, 0.76); font-size: 12px; font-weight: 800; }
        .priceBullets svg { width: 15px; height: 15px; color: var(--cyan); }
        .pricePanel a { width: 100%; height: 44px; display: flex; align-items: center; justify-content: center; gap: 8px; border-radius: 15px; color: white; font-size: 13px; font-weight: 950; background: rgba(255, 255, 255, 0.12); border: 1px solid rgba(255, 255, 255, 0.16); transition: 0.25s ease; }
        .pricePanel a:hover { background: var(--blue); }
        .pricePanel a svg { width: 15px; height: 15px; }

        .gorkiPanel { min-height: 302px; border-radius: 34px; padding: 18px; background: rgba(255, 255, 255, 0.84); border: 1px solid rgba(11, 99, 255, 0.15); box-shadow: var(--shadow); overflow: hidden; position: relative; }
        .gorkiPanel::before { content: ""; position: absolute; width: 190px; height: 190px; border-radius: 50%; right: -80px; top: -70px; background: rgba(11, 99, 255, 0.08); }
        .gorkiTop { display: flex; align-items: center; gap: 11px; position: relative; z-index: 2; margin-bottom: 10px; }
        .gorkiAvatar { width: 45px; height: 45px; border-radius: 16px; display: grid; place-items: center; background: #050914; color: white; box-shadow: 0 16px 28px rgba(7, 20, 47, 0.16); }
        ...
        .gorkiImageWrap img { width: 128px; height: 128px; object-fit: contain; filter: drop-shadow(0 14px 18px rgba(7, 20, 47, 0.12)); animation: gorkiFloat 5.2s ease-in-out infinite; }
        .gorkiMessages { position: relative; z-index: 2; display: grid; gap: 9px; }
        .gorkiMessage { min-height: 50px; padding: 12px; border-radius: 17px; color: #1d2939; background: #f7fbff; border: 1px solid rgba(11, 99, 255, 0.1); font-size: 13px; line-height: 1.38; font-weight: 830; }
        ...
        .featureCard { min-height: 154px; padding: 22px; display: flex; align-items: center; gap: 15px; border-radius: 28px; background: rgba(255, 255, 255, 0.78); border: 1px solid rgba(11, 99, 255, 0.14); box-shadow: 0 20px 44px rgba(7, 20, 47, 0.08); transition: 0.3s ease; }
        .featureCard:hover { transform: translateY(-8px); border-color: rgba(11, 99, 255, 0.34); box-shadow: 0 30px 60px rgba(7, 20, 47, 0.13); }
        .featureIcon { width: 52px; height: 52px; flex: 0 0 auto; border-radius: 18px; display: grid; place-items: center; color: white; background: linear-gradient(135deg, var(--blue), var(--blue2)); box-shadow: 0 16px 28px rgba(11, 99, 255, 0.22); }
        .featureIcon svg { width: 27px; height: 27px; }
        .featureCard h3 { font-size: 18px; letter-spacing: -0.55px; line-height: 1.1; margin-bottom: 8px; }
        .featureCard p { color: var(--muted); font-size: 13px; line-height: 1.42; }

        .bottomBand { max-width: 1500px; margin: 20px auto 0; display: grid; grid-template-columns: 1fr 0.95fr 1.35fr; gap: 16px; position: relative; z-index: 2; }
        .domainCard, .instagramCard, .trustCard { min-height: 112px; border-radius: 28px; background: white; border: 1px solid rgba(11, 99, 255, 0.14); box-shadow: 0 20px 44px rgba(7, 20, 47, 0.08); }
        .domainCard, .instagramCard { display: flex; align-items: center; gap: 18px; padding: 22px 25px; transition: 0.28s ease; }
        .domainCard:hover, .instagramCard:hover { transform: translateY(-6px); border-color: rgba(11, 99, 255, 0.32); }
        ...
        .trustCard div { display: flex; align-items: center; justify-content: center; gap: 11px; border-radius: 20px; background: #f7fbff; border: 1px solid rgba(11, 99, 255, 0.1); color: #1d2939; font-size: 15px; font-weight: 900; transition: 0.25s ease; }
        .trustCard div:hover { transform: translateY(-4px); background: white; border-color: rgba(11, 99, 255, 0.26); }
        .trustCard svg { width: 25px; height: 25px; color: var(--blue); flex: 0 0 auto; }

        .floatingTopButton { position: fixed; right: 18px; bottom: 18px; z-index: 1000; width: 46px; height: 46px; display: grid; place-items: center; border-radius: 999px; color: white; background: linear-gradient(135deg, var(--blue), var(--blue2)); box-shadow: 0 14px 26px rgba(11, 99, 255, 0.24); font-size: 22px; font-weight: 950; border: 1px solid rgba(255, 255, 255, 0.4); transition: 0.2s ease; }
        .floatingTopButton:hover { transform: translateY(-4px); box-shadow: 0 24px 46px rgba(11, 99, 255, 0.34); }
        ...
        @media (max-width: 1460px) {
          ...
          .heroShowcase { min-height: 720px; grid-template-columns: minmax(560px, 1fr) 252px; }
          .featureGrid { grid-template-columns: repeat(3, 1fr); }
          .bottomBand { grid-template-columns: 1fr; }
          .trustCard { min-height: 112px; }
        }

        @media (max-width: 1080px) {
          html, body { overflow-x: hidden; overflow-y: auto; }
          .takipioPremium { overflow-x: clip; overflow-y: visible; padding: 22px 16px 92px; }
          .navLinks { display: none !important; }
          .navBar { width: 100%; height: auto; top: 12px; left: 0; transform: none; padding: 0; background: transparent; border: 0; box-shadow: none; justify-content: center; }
          .brandCapsule { width: 170px; height: 54px; margin: 0 auto; border-radius: 22px; background: linear-gradient(180deg, rgba(7, 15, 32, 0.96), rgba(5, 9, 20, 0.96)), #050914; border: 1px solid rgba(255, 255, 255, 0.14); box-shadow: 0 18px 38px rgba(6, 16, 31, 0.18), inset 0 0 22px rgba(11, 99, 255, 0.1); justify-content: center; }
          .brandCapsule img { width: 138px; height: auto; display: block; margin: 0 auto; filter: drop-shadow(0 0 10px rgba(11, 99, 255, 0.28)); }
          .mobileNav { position: fixed; left: 12px; right: 12px; bottom: 12px; z-index: 1001; height: 58px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 7px; padding: 7px; border-radius: 24px; background: rgba(255, 255, 255, 0.96); border: 1px solid rgba(11, 99, 255, 0.12); box-shadow: 0 16px 34px rgba(15, 32, 64, 0.14); }
          .mobileNav a { display: flex; align-items: center; justify-content: center; border-radius: 17px; color: #1d2939; background: #f7fbff; border: 1px solid rgba(11, 99, 255, 0.08); font-size: 12px; font-weight: 900; }
          .mobileNav a:last-child { color: white; background: linear-gradient(135deg, var(--blue), var(--blue2)); box-shadow: 0 10px 20px rgba(11, 99, 255, 0.2); }
          .floatingTopButton { right: 18px; bottom: 82px; }
          h1 { letter-spacing: -3px; }
          .heroText { font-size: 18px; }
          .heroShowcase { grid-template-columns: 1fr; min-height: auto; }
          .demoArea { min-height: 570px; }
          .sideStack { grid-template-columns: 1fr 1fr; }
          .pricePanel, .gorkiPanel { min-height: 300px; }
          .floatRevenue { right: 26px; top: 40px; }
          .floatOrders { left: 8px; }
          .floatStock { left: 14px; }
          .featureGrid { grid-template-columns: repeat(2, 1fr); }
        }

        @media (max-width: 780px) {
          .hero { min-height: auto; gap: 24px; }
          .statusPill { font-size: 12px; white-space: normal; line-height: 1.3; }
          .waitlistHeader { align-items: flex-start; }
          .formRow { grid-template-columns: 1fr; }
          .proofBar { grid-template-columns: 1fr; }
          .demoArea { min-height: 500px; overflow: visible; padding-top: 24px; }
          .ambientGlow { width: 520px; height: 330px; }
          .laptopMock { width: 98%; height: 315px; padding: 11px 11px 22px; border-radius: 24px 24px 16px 16px; }
          .laptopMock::before { width: 52px; height: 8px; }
          .screenTop { height: 44px; padding: 0 12px; }
          .screenBrand { font-size: 12px; }
          .screenBrand img { width: 18px; height: 18px; }
          .screenBody { height: calc(100% - 44px); grid-template-columns: 86px 1fr; }
          .mockMenu { padding: 10px 7px; gap: 7px; }
          .mockMenu button { min-height: 28px; font-size: 0; }
          .mockMenu button::after { content: ""; display: block; width: 70%; height: 7px; border-radius: 999px; background: currentColor; margin: auto; opacity: 0.65; }
          .mockAssistant { display: none; }
          .mockContent { padding: 10px; }
          .mockHeader { margin-bottom: 8px; }
          .mockHeader h3 { font-size: 13px; }
          .mockHeader span { font-size: 9px; }
          .mockCards { grid-template-columns: 1fr; gap: 8px; }
          .mockCards .mockCard:nth-child(2), .mockCards .mockCard:nth-child(3), .mockCards .mockCard:nth-child(4) { display: none; }
          .mockCard { min-height: 58px; padding: 10px; }
          .mockLower { grid-template-columns: 1fr; }
          .mockGraph { height: 112px; }
          .activityPanel, .mockList { display: none; }
          .phoneMock { width: 124px; height: 250px; right: -4px; bottom: 16px; border-radius: 28px; padding: 8px; }
          .phoneScreen { border-radius: 21px; padding: 10px 8px; }
          .phoneGreeting, .phoneTabs { display: none; }
          .phoneGrid { grid-template-columns: 1fr; }
          .phoneGrid .phoneCard:nth-child(3), .phoneGrid .phoneCard:nth-child(4) { display: none; }
          .phoneList div { height: 26px; font-size: 0; }
          .floatingCard { display: none; }
          .sideStack { grid-template-columns: 1fr; }
          .pricePanel, .gorkiPanel { min-height: auto; }
          .featureGrid { grid-template-columns: 1fr; }
          .featureCard { min-height: 132px; }
          .domainCard, .instagramCard { align-items: flex-start; }
          .domainCard h3, .instagramCard h3 { font-size: 26px; }
          .trustCard { grid-template-columns: 1fr; }
          .trustCard div { min-height: 62px; }
        }

        @media (max-width: 460px) {
          .takipioPremium { padding: 18px 12px 92px; }
          h1 { font-size: 45px; line-height: 1; letter-spacing: -2.4px; }
          .heroText { font-size: 16px; }
          .waitlistCard { padding: 18px; border-radius: 24px; }
          .waitlistHeader h2 { font-size: 21px; }
          .mailIcon { width: 48px; height: 48px; border-radius: 16px; }
          .proofBar div { min-height: 74px; }
          .demoArea { min-height: 455px; }
          .laptopMock { height: 285px; }
          .phoneMock { scale: 0.9; right: -20px; bottom: 8px; }
          .priceNumbers strong { font-size: 62px; }
          .priceNumbers del { font-size: 36px; }
        }
      `}</style>
    </main>
  );
      ...
      <div className="laptopScreen">
        ...
        <div className="screenBody">
          <div className="mockMenu">
            ...
            <div className="mockAssistant"><b>Gorki AI</b><span>Asistanın hazır. Bugünkü işlerini özetliyorum.</span></div>
          </div>
          <div className="mockContent">
            <div className="mockHeader"><h3>{active.label}</h3><span>Canlı demo modu</span></div>
            <div className="mockCards">
              <div className="mockCard"><small>{active.helper}</small><b>{active.value}</b><em>+%18,6</em></div>
              <div className="mockCard"><small>Aktif işlem</small><b>{activeTab === "orders" ? "24" : activeTab === "payments" ? "8" : "18"}</b><em>Bugün</em></div>
              <div className="mockCard"><small>Müşteri</small><b>89</b><em>+%5,7</em></div>
              <div className="mockCard"><small>Durum</small><b>{activeTab === "payments" ? "Takipte" : "Güncel"}</b><em>Aktif</em></div>
            </div>
            <div className="mockLower">
              ...
              <div className="activityPanel">
                <h4>Canlı Akış</h4>
                <div><span>Yeni sipariş #10248</span><b>2 dk</b></div>
                <div><span>Ödeme alındı</span><b>15 dk</b></div>
                <div><span>Stok güncellendi</span><b>1 sa</b></div>
                <div><span>Müşteri kaydı</span><b>2 sa</b></div>
              </div>
            </div>
  ...
  return (
    <div className="phoneMock">
      <div className="phoneScreen">
        ...
        <div className="phoneGreeting"><b>Merhaba, Ahmet 👋</b><span>Bugün nasıl gidiyor?</span></div>
        <div className="phoneTabs">
          {demoTabs.map((tab) => (
            <button key={tab.key} type="button" className={activeTab === tab.key ? "active" : ""} onClick={() => setActiveTab(tab.key)}>{tab.short}</button>
          ))}
        </div>
        <div className="phoneGrid">
          <div className="phoneCard"><small>{active.label}</small><b>{active.value}</b><em>+%18,6</em></div>
          <div className="phoneCard"><small>Sipariş</small><b>128</b><em>+%8,2</em></div>
          <div className="phoneCard"><small>Müşteri</small><b>89</b><em>+%5,7</em></div>
          <div className="phoneCard"><small>Stok</small><b>Güncel</b><em>128 ürün</em></div>
        </div>
        <div className="phoneList">
          <div><span>#10248 Ahmet</span><b>Hazırlanıyor</b></div>
          <div><span>#10247 Zeynep</span><b>Kargoda</b></div>
          <div><span>#10246 Mehmet</span><b>Tamamlandı</b></div>
        </div>
      </div>
    </div>
  );
...
function PricePanel() {
  return (
    <div className="pricePanel" id="pricing">
      ...
      <p>İlk ay sadece</p>
      <div className="priceNumbers"><del>₺99</del><strong>₺89</strong></div>
      ...
      <div className="priceBullets">
        ...
        <span><CheckIcon /> 7/24 destek</span>
      </div>
  ...
  return (
    <div className="gorkiPanel" id="assistant">
      <div className="gorkiTop">
        <div className="gorkiAvatar"><img src="/gorki-hero.png" alt="" /></div>
      ...
      <div className="gorkiFooter"><HeartIcon /> Gorki her zaman yanında.</div>
    </div>
  );
}

function FloatingInfoCard({ icon, title, text, metric, className }: { icon: React.ReactNode; title: string; text: string; metric: string; className: string }) {
  return (
    <div className={`floatingCard ${className}`}>
      {icon}
      <div><b>{title}</b><span>{text}</span><em>{metric}</em></div>
    </div>
  );
}
