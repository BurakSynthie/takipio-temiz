...
export default function SettingsPage() {
  ...
  async function deleteMember(member: Member) {
    ...
    if (!confirm(`${member.email} işletmeden tamamen kaldırılsın mı? Profili, davetleri ve mümkünse Auth hesabı da silinecek.`)) return;
    ...
    const response = await fetch("/api/team-members/hard-delete", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!response.ok) {
      setMessage(payload.error || "Ekip üyesi silinemedi.");
      setSaving(false);
      return;
    }

    setMessage(payload.message || "Ekip üyesi tamamen silindi.");
